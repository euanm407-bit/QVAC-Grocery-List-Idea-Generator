#!/usr/bin/env node
// QVAC Grocery List Idea Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29383;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/grocerylist") {
      try {
        ...
        const result = await generate(modelId, meal.trim());
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Grocery List Idea Generator GUI ready at http://localhost:${PORT}`);
  });
